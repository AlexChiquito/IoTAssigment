...
public class ProviderApplicationInitListener extends ApplicationInitListener {
	...
	protected void customInit(final ContextRefreshedEvent event) {
		...
		//System.out.println(Base64.getEncoder().encodeToString(arrowheadService.getMyPublicKey().getEncoded()));
		
		//Register services into ServiceRegistry
		final ServiceRegistryRequestDTO getLastPointsService = createServiceRegistryRequest("get-npoints", "/GetLastPoints", HttpMethod.GET);
		logger.info("Created Request");	
		getLastPointsService.getMetadata().put("request-param-n", "nPoints");
		logger.info("added param");
		arrowheadService.forceRegisterServiceToServiceRegistry(getLastPointsService);
		logger.info("Forcer register");
